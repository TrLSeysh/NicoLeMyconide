...
class Game:
    ...
    def update_screen(self) -> None:
        ...
        if self.current_screen == "home_screen":
            ...
            self.window.display_screen([self.buttons[self.current_screen], self.bars, self.dynamic])
    ...
    def is_button_pressed(self, current_btn: dict) -> Button or None:
        """
        ...
        for button in current_btn:
            if (
                current_btn[button].coordinates[0]
                < m_pos[0]
                < current_btn[button].coordinates[0] + current_btn[button].size[0]
            ):
                if (
                    current_btn[button].coordinates[1]
                    < m_pos[1]
                    < current_btn[button].coordinates[1] + current_btn[button].size[1]
                ):
    ...
    def is_element_clicked(self, elements: dict, *args) -> Item or None:
        """
        ...
        for name in elements:
            if (
                elements[name].coordinates[0]
                < m_pos[0]
                < elements[name].coordinates[0] + elements[name].size[0]
            ):
                if (
                    elements[name].coordinates[1]
                    < m_pos[1]
                    < elements[name].coordinates[1] + elements[name].size[1]
                ):
                    if args:
    ...
    def has_button_been_pressed(self, current_btn: dict) -> Button or None:
        """
    ...
    def mouse_on_nico(self, m_pos: tuple) -> bool:
        """
    ...
    def home_screen_events(self, ev: pg.event) -> bool:
        """
    ...
    def main_screen_events(self, ev: pg.event) -> bool:
        """
        ...
        if self.current_action["type"] == "pet":
            ...
            if self.mouse_on_nico(m_pos):
                if (
                    sqrt(
                        (self.old_mouse_pos[0] - m_pos[0]) ** 2
                        + (self.old_mouse_pos[1] - m_pos[1]) ** 2
                    )
                    > 20
                ):
                    ...
                    if not self.nico.pet():
                        ...
                        # Change heart texture state each five iteration to slowdown animation
                        if not self.petted_count % 5:
                            self.current_action["state"] = (
                                self.current_action["state"] + 1
                                if self.current_action["state"] + 1 < 4
                                else 0
                            )
                        ...
                        self.window.display_anim(
                            [
                                self.nico.coordinates[0] + self.nico.size[0] + 10,
                                self.nico.coordinates[1] - 30,
                            ],
                            [42, 74],
                            self.current_action["state"],
                            "hearts",
                ...
                # Remove hearts if mouse is outside Nico's hitbox
                self.window.display_anim(
                    [
                        self.nico.coordinates[0] + self.nico.size[0] + 10,
                        self.nico.coordinates[1] - 30,
        ...
        if ev.type == pg.MOUSEBUTTONDOWN:
            ...
            if button:
                if button == "menu_button":
                    ...
                    # This button is only displayed when petting mode is active.
                    # When clicked it disappear and disable petting mode
                    pg.mouse.set_cursor(pg.SYSTEM_CURSOR_ARROW)
                    self.window.remove_element_from_screen(
                        self.buttons["main_screen"]["stop_button"].coordinates,
                        self.buttons["main_screen"]["stop_button"].size,
                    )
                ...
                if element_name == "food":
                    if self.dynamic["food"].state == 0:
                        # If Nico wants to eat, feed him and delete food, else reset food state waiting another click
    ...
    def menu_screen_events(self, ev: pg.event) -> bool:
        """
        ...
        if ev.type == pg.MOUSEBUTTONDOWN:
            ...
            if button:
                if button == "home_button":
                    ...
                    # Change current action to pet mode. it changes cursor and allow player to pet Nico
                    pg.mouse.set_cursor(pg.SYSTEM_CURSOR_HAND)
                    # Add a button on screen to disable pet mode
                    self.buttons["main_screen"]["stop_button"] = Button(
                        [447, 237], [32, 32], "stop_button"
                    )
    ...
    def start_game(self) -> None:
        ...
        # Select current screen and display
        self.current_screen = "main_screen"
        self.update_screen()
    ...
    def running_game(self) -> None:
        ...
        if self.current_screen == "main_screen":
            ...
            # Animate "Zzz" if Nico is sleeping
        ...
        # Change Nico state on time elapsed. It changes energy and other attributes as well as bars
